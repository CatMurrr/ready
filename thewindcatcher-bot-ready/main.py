...
async def init_db():
    async with aiosqlite.connect(DB) as db:
        ...
        await db.execute("INSERT OR IGNORE INTO hunt(rowid,prey,last_spawn) VALUES(1,6,?)", (utc_now,))
        await db.execute("INSERT OR IGNORE INTO herbs(rowid,available,last_spawn) VALUES(1,5,?)", (utc_now,))
        await db.commit()
...
class MyBot(commands.Bot):
    ...
    async def setup_hook(self):
        await init_db()
        self.loop.create_task(monitor_status_task())
        self.loop.create_task(spawn_prey_task())
        self.loop.create_task(spawn_herbs_task())
